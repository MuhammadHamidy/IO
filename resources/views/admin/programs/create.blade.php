...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-gray-600">Add a new educational program</p>
        </div>
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <select id="type" name="type" required class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('type') border-red-500 @enderror">
                    ...
                    <option value="non-degree" {{ old('type')==='non-degree' ? 'selected' : '' }}>Non-Degree</option>
                </select>
                    @error('type')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <select id="program_type" name="program_type" class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('program_type') border-red-500 @enderror">
                        ...
                        <option value="inbound" {{ old('program_type')==='inbound' ? 'selected' : '' }}>Inbound</option>
                        <option value="outbound" {{ old('program_type')==='outbound' ? 'selected' : '' }}>Outbound</option>
                    </select>
                ...
                <div>
                    ...
                    <select id="category" name="category" class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('category') border-red-500 @enderror">
                        ...
                        <option value="UPER-SA" {{ old('category')==='UPER-SA' ? 'selected' : '' }}>UNIVERSITAS PERTAMINA STUDY ABROAD (UPER-SA)</option>
                        <option value="Student Exchange Program" {{ old('category')==='Student Exchange Program' ? 'selected' : '' }}>Student Exchange Program</option>
                        <option value="Internship/Research Attachment" {{ old('category')==='Internship/Research Attachment' ? 'selected' : '' }}>Internship/Research Attachment</option>
                        <option value="Short-term Program" {{ old('category')==='Short-term Program' ? 'selected' : '' }}>Short-term Program</option>
                    </select>
                    @error('category')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
            ...
            <div>
                ...
                <textarea id="requirements" name="requirements" rows="4" required
                          ...
                          placeholder="Enter program requirements...">{{ old('requirements') }}</textarea>
                @error('requirements')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="datetime-local" id="open_date" name="open_date" value="{{ old('open_date') }}"
                ...
                <div>
                    ...
                    <input type="datetime-local" id="close_date" name="close_date" value="{{ old('close_date') }}"
            ...
            <div>
                ...
                <p class="mt-1 text-sm text-gray-500">Upload program image (optional)</p>
                @error('image')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
