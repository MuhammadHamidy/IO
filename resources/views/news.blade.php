<x-layout>
    <div class="w-full bg-gradient-to-b from-gray-50 to-white">
        ...
        <div class="max-w-screen-2xl mx-auto px-6 py-10">
            ...
            <div class="mb-6">
                <a href="{{ route('home') }}" class="inline-flex items-center gap-2 px-4 py-2 bg-white border-2 border-gray-300 text-gray-700 rounded-lg hover:border-[#1F4894] hover:text-[#1F4894] hover:shadow-md transition-all duration-300 font-semibold group">
                ...
                slides: [
                    @isset($highlightedNews)
                        @foreach($highlightedNews as $news)
                            {
                                id: {{ $news->id }},
                                img: '{{ $news->cover ? asset('storage/' . $news->cover) : ($news->image ? asset('storage/' . $news->image) : asset('img/default-news.png')) }}',
                                title: {{ json_encode($news->title) }},
                                excerpt: {{ json_encode(\Illuminate\Support\Str::limit(strip_tags($news->content), 350, '...')) }}
                            },
                        @endforeach
                    @endisset
                ],
                ...
                <div class="bg-white rounded-2xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1">
                    <div class="relative h-72 overflow-hidden">
                        @if($news->cover || $news->image)
                            <img src="{{ $news->cover ? asset('storage/' . $news->cover) : asset('storage/' . $news->image) }}" alt="{{ $news->title }}" class="w-full h-full object-cover transform hover:scale-110 transition-transform duration-500">
                        @else
                    ...
                    <div class="p-8">
                        ...
                        <p class="text-gray-600 mb-4 leading-relaxed line-clamp-3">{!! \Illuminate\Support\Str::limit(strip_tags($news->content, '<b><i><strong><em>'), 200, '...') !!}</p>
                        <a href="{{ route('news-detail', $news->id) }}" class="inline-flex items-center text-blue-600 font-semibold hover:text-blue-800 transition-colors">
                            Read More 
        ...
        {{-- Events section above News Archive --}}
                    ...
                    @foreach($events as $event)
                        <div class="bg-white rounded-xl overflow-hidden shadow-md hover:shadow-xl transition-all duration-300 border border-gray-100">
                            ...
                            <div class="p-6">
                                ...
                                <h3 class="text-xl font-bold mb-3 text-gray-900 line-clamp-2">{{ $event->title ?? $event->name }}</h3>
                                @if($event->description)
                                <p class="text-gray-600 text-sm mb-4 line-clamp-2">{{ \Illuminate\Support\Str::limit(strip_tags($event->description), 120, '...') }}</p>
                                @endif
                                <a href="{{ route('events.show', $event->id) }}" class="text-green-700 text-sm font-semibold hover:text-green-900 transition-colors">
                                    Read More →
                                </a>
                            </div>
                        </div>
                    ...
                    @foreach($oldNews as $news)
                        <div class="bg-white rounded-xl overflow-hidden shadow-md hover:shadow-xl transition-all duration-300">
                            <div class="relative h-48 overflow-hidden">
                                @if($news->cover || $news->image)
                                    <img src="{{ $news->cover ? asset('storage/' . $news->cover) : asset('storage/' . $news->image) }}" alt="{{ $news->title }}" class="w-full h-full object-cover transform hover:scale-105 transition-transform duration-300">
                                @else
                            ...
                            <div class="p-6">
                                ...
                                <p class="text-gray-600 text-sm mb-4 line-clamp-2">{!! \Illuminate\Support\Str::limit(strip_tags($news->content, '<b><i><strong><em>'), 120, '...') !!}</p>
                                <a href="{{ route('news-detail', $news->id) }}" class="text-blue-600 text-sm font-semibold hover:text-blue-800 transition-colors">
                                    Read More →
                                </a>
                            </div>
                        </div>
        ...
        <div class="bg-blue-50 py-12">
            <div class="max-w-screen-2xl mx-auto px-6">
                <div class="mb-8 flex justify-between items-center">
                    ...
                    <a href="{{ route('regist-news') }}" class="bg-gradient-to-r from-blue-600 to-purple-600 text-white rounded-xl px-6 py-3 hover:shadow-lg transform hover:scale-105 transition-all duration-300 font-semibold flex items-center">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    @foreach($allNews as $news)
                        <div class="bg-white rounded-xl overflow-hidden shadow-md hover:shadow-xl transition-all duration-300">
                            ...
                            @endif
                            <div class="p-6">
                                <h3 class="text-xl font-bold mb-3 text-gray-900 line-clamp-2">{{ $news->title }}</h3>
                                <p class="text-gray-600 text-sm mb-4 line-clamp-2">{!! \Illuminate\Support\Str::limit(strip_tags($news->content, '<b><i><strong><em>'), 120, '...') !!}</p>
                                <div class="flex gap-2 mt-4">
                                    <a href="{{ route('news-update', $news->id) }}" class="flex-1 bg-blue-500 text-white text-center rounded-lg px-4 py-2 hover:bg-blue-600 transition-colors font-medium">
                                        Edit
                                    </a>
                                    <button onclick="openModal({{ $news->id }})" class="flex-1 bg-red-500 text-white rounded-lg px-4 py-2 hover:bg-red-600 transition-colors font-medium">
                                        Delete
                                    </button>
                                </div>
                            </div>
