...
<div class="max-w-6xl mx-auto py-10 px-4 mt-4">
    ...
    @if(request()->get('back') === 'notifications')
        <div class="mb-4">
            <a href="{{ route('notifications.index') }}" class="inline-flex items-center text-blue-600 hover:text-blue-800 font-medium transition">
    ...
    @else
        <div class="mb-4">
            <a href="{{ route('user.applications.index') }}" class="inline-flex items-center text-blue-600 hover:text-blue-800 font-medium transition">
                <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path>
                </svg>
    ...
    <div class="bg-gradient-to-r from-blue-600 to-indigo-700 text-white rounded-2xl p-8 mb-8 shadow-2xl">
        ...
        <div>
                ...
                <p class="text-blue-100 text-lg">{{ $application->program->title ?? 'Program' }}</p>
            </div>
            <div class="text-right">
                <div class="text-sm text-blue-100 mb-1">Application Progress</div>
                <div class="text-4xl font-bold">{{ $application->getProgressPercentage() }}%</div>
            </div>
        </div>
    </div>
            ...
            <div class="bg-white rounded-2xl shadow-xl border border-gray-100 p-8">
                ...
                <div class="mb-8">
                    <div class="flex justify-between items-center mb-2">
                        ...
                        <span class="text-sm font-bold text-blue-600">{{ $application->getProgressPercentage() }}%</span>
                    </div>
                    <div class="w-full bg-gray-200 rounded-full h-3">
                        <div class="bg-gradient-to-r from-blue-500 to-indigo-600 h-3 rounded-full transition-all duration-500" style="width: {{ $application->getProgressPercentage() }}%"></div>
                    </div>
                </div>
                    ...
                    @foreach($stages as $stageKey => $stageLabel)
                        ...
                        <div class="flex items-start">
                            ...
                            <div class="flex-shrink-0 mr-4">
                                ...
                                @endphp
                                @if($isCompleted || $isCompletedStageIcon)
                            ...
                            <div class="flex-1 pb-8 {{ !$isPending ? 'border-l-2 border-gray-200 pl-4 ml-5' : 'pl-4 ml-5' }}">
                                <div class="flex items-center justify-between mb-1">
                                    ...
                                    @endphp
                                    <h4 class="font-bold text-lg {{ $titleClass }}">
                                        {{ $stageLabel }}
                                    </h4>
                                ...
                                @if($isCurrent)
                                    <p class="text-sm text-gray-600 mt-2">
                                        @if(in_array($stageKey, ['submitted', 'waiting_approval_1', 'waiting_approval_2']))
                                            Your application is currently under review. Please wait for the admin's decision.
                                        @elseif($stageKey === 'upload_requirements')
                                            Please upload all required documents to proceed.
                                        @elseif($stageKey === 'globalization_announcement')
                                            Check your email for globalization program announcement.
                                        @elseif($stageKey === 'upload_transcript')
                                            Upload your transcript and related documentation.
                                        @elseif($stageKey === 'survey')
                        ...
                        @foreach($profileDocs as $doc)
                            @if(auth()->user()->{$doc['field']})
                                <li class="flex items-center justify-between bg-green-50 border-2 border-green-200 p-3 rounded-lg">
                                    ...
                                    <a href="{{ asset('storage/' . auth()->user()->{$doc['field']}) }}" target="_blank" class="text-blue-600 hover:text-blue-700 font-semibold flex items-center text-sm">
                                ...
                                @foreach($additionalDocs as $doc)
                                    <li class="flex items-center justify-between bg-blue-50 border-2 border-blue-200 p-3 rounded-lg">
                                        <div class="flex items-center">
                                            ...
                                            <span class="font-medium text-gray-700">{{ basename($doc) }}</span>
                                        </div>
                                        <a href="{{ asset('storage/' . $doc) }}" target="_blank" class="text-blue-600 hover:text-blue-700 font-semibold flex items-center text-sm">
                                            <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path>
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path>
                                            </svg>
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white rounded-2xl shadow-xl border border-gray-100 p-6">
                ...
                <div class="space-y-3">
                    ...
                    <div>
                        ...
                        <div class="font-semibold text-gray-900">{{ $application->submitted_at ? $application->submitted_at->format('d M Y, H:i') : '-' }}</div>
                    </div>
                    @if($application->reviewed_at)
                        <div>
                            <div class="text-sm text-gray-500">Last Reviewed</div>
                            <div class="font-semibold text-gray-900">{{ $application->reviewed_at->format('d M Y, H:i') }}</div>
                        </div>
                    @endif
                    @if($application->stage_updated_at)
                        <div>
                            <div class="text-sm text-gray-500">Stage Updated</div>
                            <div class="font-semibold text-gray-900">{{ $application->stage_updated_at->format('d M Y, H:i') }}</div>
                        </div>
                    @endif
                </div>
            </div>
            ...
            @if($application->admin_notes)
                <div class="bg-gradient-to-br from-yellow-50 to-orange-50 border-l-4 border-yellow-500 rounded-r-2xl shadow-xl p-6">
                    ...
                    <p class="text-gray-700 leading-relaxed">{{ $application->admin_notes }}</p>
                </div>
            @endif
            ...
            @if($application->notes)
                <div class="bg-white rounded-2xl shadow-xl border border-gray-100 p-6">
                    ...
                    <p class="text-gray-700 leading-relaxed">{{ $application->notes }}</p>
                </div>
            @endif

            
            <div class="bg-white rounded-2xl shadow-xl border border-gray-100 p-6">
                <a href="{{ route('user.applications.index') }}" class="block w-full text-center bg-gray-600 text-white px-6 py-3 rounded-xl font-semibold hover:bg-gray-700 transition-all duration-300 shadow-lg">
