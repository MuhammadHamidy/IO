...
<div class="min-h-screen bg-gray-50 py-8">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        @forelse($notifications as $notif)
                ...
                <button 
                    onclick="openNotificationModal{{ $loop->index }}()" 
                    class="w-full text-left bg-white rounded-lg shadow hover:shadow-md transition-all duration-200 p-4 border-l-4 {{ $isUnread ? 'border-blue-500 bg-blue-50' : 'border-gray-300' }}">
                <div class="flex items-center justify-between">
                        <div class="flex items-center gap-3 flex-1">
                            <div class="w-10 h-10 rounded-full {{ $isUnread ? 'bg-blue-500' : 'bg-gray-400' }} flex items-center justify-center flex-shrink-0">
                            ...
                            <div class="flex-1 min-w-0 overflow-hidden">
                                <div class="flex items-center gap-2 flex-wrap">
                                    <h3 class="font-semibold text-gray-900 break-words line-clamp-1">{{ $programName }}</h3>
                                    @if($isUnread)
                                        <span class="inline-block px-2 py-0.5 text-xs font-semibold bg-blue-500 text-white rounded-full flex-shrink-0">New</span>
                        @endif
                                </div>
                                <p class="text-sm text-gray-600 mt-1 break-words line-clamp-2">{{ $message }}</p>
                ...
                <div id="notificationModal{{ $loop->index }}" class="fixed inset-0 bg-black bg-opacity-50 z-[200] flex items-center justify-center p-4" style="display: none;">
                    <div class="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
                        ...
                        <div class="sticky top-0 bg-gradient-to-r from-blue-600 to-purple-600 text-white p-6 rounded-t-2xl">
                            <div class="flex items-start justify-between">
                                <div>
                                    <h2 class="text-2xl font-bold">{{ $programName }}</h2>
                                    <p class="text-blue-100 mt-1">{{ $notif->created_at->format('F d, Y - H:i') }}</p>
                                </div>
                                <button onclick="closeNotificationModal{{ $loop->index }}()" class="text-white hover:text-gray-200 transition">
                        ...
                        <div class="p-6 space-y-6">
                            ...
                            <div>
                                ...
                                <p class="text-gray-800 text-lg leading-relaxed break-words whitespace-pre-wrap">{{ $message }}</p>
                            </div>
                            ...
                            <div class="bg-yellow-50 border-l-4 border-yellow-400 p-4 rounded-r-lg">
                                ...
                                <p class="text-yellow-900 break-words whitespace-pre-wrap">{{ $adminNotes }}</p>
                            </div>
                            @endif
                        </div>
                        ...
                        <div class="bg-gray-50 px-6 py-4 rounded-b-2xl flex items-center justify-between gap-3">
                            @if($isUnread)
                            <form method="POST" action="{{ route('notifications.mark-read', $notif->id) }}" class="flex-1">
                        @csrf
                            ...
                            @endphp
                            @if($applicationId)
                            <a href="{{ route('user.applications.show', $applicationId) }}?back=notifications" class="flex-1 px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 transition font-medium text-center flex items-center justify-center gap-2">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <script>
                    function openNotificationModal{{ $loop->index }}() {
                        document.getElementById('notificationModal{{ $loop->index }}').style.display = 'flex';
                        document.body.style.overflow = 'hidden';
                    }
                    
                    function closeNotificationModal{{ $loop->index }}() {
                        document.getElementById('notificationModal{{ $loop->index }}').style.display = 'none';
                        document.body.style.overflow = 'auto';
                    }

                    document.getElementById('notificationModal{{ $loop->index }}').addEventListener('click', function(e) {
                        if (e.target === this) {
                            closeNotificationModal{{ $loop->index }}();
                        }
                    });
                </script>
            @empty
