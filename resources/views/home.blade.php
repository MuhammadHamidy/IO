        ...
        <div class="w-full bg-gradient-to-b from-white to-gray-50 py-16">
            <div class="max-w-full mx-auto px-4">
                <div class="flex items-center justify-between mb-10 max-w-7xl mx-auto">
                    ...
                    <a href="{{ route('news') }}" class="px-6 py-3 bg-[#1F4894] text-white rounded-lg hover:bg-[#163872] transition-all duration-300 font-semibold flex items-center gap-2 shadow-lg hover:shadow-xl">
                    ...
                    <div x-data="{
                        slides: [
                            @foreach($recentNews as $news)
                                {
                                    id: {{ json_encode($news->id) }},
                                    image: '{{ $news->cover ? asset('storage/' . $news->cover) : ($news->image ? asset('storage/' . $news->image) : '') }}',
                                    title: {{ json_encode($news->title) }},
                                    excerpt: {{ json_encode(\Illuminate\Support\Str::limit(strip_tags($news->content), 120, '...')) }},
                                    date: '{{ $news->created_at->format('M d, Y') }}',
                                    isHighlight: {{ $news->is_highlight ? 'true' : 'false' }}
                                },
                            @endforeach
                        ],
                        current: 0,
            ...
            <div class="gap-4 flex flex-col items-center w-full">
                ...
                @if(isset($programs) && count($programs) > 0)
                        ...
                        <div x-data="{
                            slides: [
                                @foreach($programs as $program)
                                    {
                                        id: {{ $program->id }},
                                        image: '{{ $program->image ? asset('storage/' . $program->image) : '' }}',
                                        title: '{{ addslashes($program->title) }}',
                                        description: '{{ addslashes(\Illuminate\Support\Str::limit(strip_tags($program->description), 150, '...')) }}',
                                        type: '{{ ucfirst($program->type ?? 'Program') }}',
                                        duration: '{{ $program->duration ?? '' }}',
                                        route: '{{ route($program->type === 'degree' ? 'programs.degree' : 'programs.non-degree') }}'
                                    },
                                @endforeach
                            ],
                            current: 0,
                    ...
                    <div class="w-full mt-6">
                        <a href="{{ route('program') }}" class="w-full flex items-center justify-center gap-2 bg-gradient-to-r from-[#1F4894] to-[#275DCB] text-white px-6 py-3 rounded-lg font-semibold hover:from-[#163872] hover:to-[#1e4da3] transition-all duration-300 shadow-md hover:shadow-lg">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
