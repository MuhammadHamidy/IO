...
<?php $component->withAttributes([]); ?>
    ...
    <div class="min-w-max min-h-max sm:min-w-screen sm:min-h-screen p-[100px] flex flex-col gap-16 items-center justify-center bg-cover" style="background-image: url('<?php echo e(asset('img/auth-background.png')); ?>')">
        <img src="<?php echo e(asset('img/logo-white.png')); ?>" class=" w-[100px] h-[60px] sm:w-[182px] sm:h-[130px]"/>
        ...
        <div class="flex flex-col px-20 py-10 bg-[#1F4894] bg-opacity-85 gap-4 sm:gap-12">
            <div class="flex flex-col items-center justify-center gap-5">
                ...
                <a href="<?php echo e(route('login-outbound')); ?>" class="text-center text-white bg-[#C4D25A] w-[120px] h-max p-2 rounded-lg text-sm min-w-max">Login with SSO</a>
                <p class="font-bold text-white text-start text-[15px] sm:text-[20px]">OR</p>
            </div>
            <h2 class="font-semibold text-white text-start text-[16px] sm:text-[22px] border-b-[#C4D25A] border-b-[3px] w-max pe-[20px]">Login</h2>
            <form class="w-full h-fit flex flex-col gap-2" method="POST" action="<?php echo e(route('login')); ?>">
                <?php echo csrf_field(); ?>
                <div class="w-full flex flex-col">
                    <label class="mb-2 mt-4 text-white">Email</label>
                ...
                <div class="w-full flex flex-row justify-between items-center">
                    ...
                    <div class="my-6 text-[14px]"><a href="" class="text-white underline">Forgot Password</a></div>
                </div>
                <div class="w-full flex flex-col justify-center items-center">
                    ...
                    <div class="my-6 text-[14px] text-white">Don't have account yet? <a href="<?php echo e(route('regist-inbound')); ?>" class="text-white underline">Register Here</a></div>
                </div>
                <script>
    ...
    <script>
        document.querySelector('form').addEventListener('submit', async function (e) {
            ...
            try {
                const response = await fetch('/api/login-inbound', {
                    method: 'POST',
                    headers: {
...
<?php endif; ?>
<?php /**PATH C:\Kuliah\laragon\www\KP\IO-web\resources\views/login-inbound.blade.php ENDPATH**/ ?>